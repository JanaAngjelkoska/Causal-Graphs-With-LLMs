...
def test_pipeline_prevents_cycle_creation():
    # Mock agent
    mock_agent = MagicMock(spec=GitHubLLMAgent)
    ...
    # Mock extraction response with a cycle attempt (Z -> X)
    def extraction_side_effect(cause, effects):
        mapping = {
            "X": ExpansionResponse(variable="X", effects=["Y"]),
            "Y": ExpansionResponse(variable="Y", effects=["Z"]),
            "Z": ExpansionResponse(variable="Z", effects=["X"])  # This would create a cycle
        }
        return mapping.get(cause, ExpansionResponse(variable=cause, effects=[]))
    ...
    # The edge Z -> X should NOT be added, so the graph remains acyclic
    expected_graph = {
        "X": ["Y"],
        "Y": ["Z"],
        "Z": []
